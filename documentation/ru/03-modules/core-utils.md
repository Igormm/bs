...
Source-guard для модулей, подключаемых вручную. Возвращает `0`, если модуль ещё **не**
загружался (можно продолжать), и `1`, если уже загружался (нужно сразу сделать `return 0`).
...
`BS_LOADED_MODULES` загрузчика; `utils::guard` предназначен для файлов, подключаемых
в обход загрузчика.
