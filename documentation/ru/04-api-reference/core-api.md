...
- `arr::from_lines <массив>` — читает строки stdin в массив через `mapfile`; используйте `arr::from_lines arr < file` или `< <(cmd)` (пайп выполнится в subshell и массив потеряется)
- `map::has <map> <ключ>` — предикат

Все принимают ИМЯ переменной (не значение), напр. `arr::join my_tools ", "`.

### `is::` — предикаты для людей

Дружелюбные замены флагов `[[ ... ]]` — не нужно помнить, что `-n` значит
«непусто», а `-f` — «обычный файл»:
...
Отрицание читается естественно: `if ! is::file "${path}"; then`.
...
#### `signal::on <сигнал> <handler>` / `signal::ignore <сигнал>` / `signal::reset <сигнал>`
- «Лингвистические» обёртки над `trap`: подписка функции на сигнал, игнорирование, возврат к поведению по умолчанию. Имена с `SIG` и без
- Возвращает: `E_INVALID` при неизвестном сигнале, `E_ERROR` если функция-обработчик не существует
- Пример: `signal::on SIGINT my::cleanup`
