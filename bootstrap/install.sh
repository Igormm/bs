#!/usr/bin/env bs
#
# BS/BS installer (Linux) / Установщик BS/BS (Linux)
#
# Режимы / Modes:
...
#
# Переопределения (опционально) / Переопределения (опционально):
...
# Функция вывода ошибки и завершения / Function to output error and exit
die() {
    printf "ERROR: %s\n" "$*" >&2
    exit 1
}

# Функция показа справки / Function to show help
usage() {
    cat << 'HELP'
BS installer
...
# Проверка прав root (для системной установки)
need_root() {
    # ${EUID:-$(id -u)}: использует EUID если определена, иначе получает через id -u
    if [[ "${EUID:-$(id -u)}" -ne 0 ]]; then
        die "Нужны права root. Запусти: sudo ./install.sh"
    fi
}
...
# Определение корневой директории исходников
